...
public class PaymentLinkRequestClient extends RequestClient {
    ...
    public HttpRequest buildDeleteRequest(String pathVariable) {
        String url = this.baseUrl + this.path + "/" + pathVariable;
        return HttpRequest.newBuilder()
                .uri(URI.create(url))
                .header("Content-Type", "application/json")
    ...
    public HttpRequest buildPostRequestWithPathVariable(String pathVariable) {
        String url = this.baseUrl + this.path + "/" + pathVariable + "/restore";
        return HttpRequest.newBuilder()
                .uri(URI.create(url))
                .header("Content-Type", "application/json")
                .header("User-Agent", "Sandbox Integration (Back-end)")
